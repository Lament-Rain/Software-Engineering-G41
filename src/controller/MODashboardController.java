...
public class MODashboardController {
    ...
    @FXML
    private TableColumn<JobViewModel, String> titleCol;
    @FXML
    private TableColumn<JobViewModel, String> departmentCol;
    @FXML
    private TableColumn<JobViewModel, String> jobTypeCol;
    @FXML
    private TableColumn<JobViewModel, String> requirementsCol;
    @FXML
    private TableColumn<JobViewModel, String> deadlineCol;
    @FXML
    private TableColumn<JobViewModel, String> statusCol;
    @FXML
    private TableColumn<JobViewModel, Integer> appCountCol;
    ...
    private void setupTableColumns() {
        ...
        if (jobTypeCol != null) {
            jobTypeCol.setCellValueFactory(new javafx.scene.control.cell.PropertyValueFactory<>("jobType"));
        }
        if (requirementsCol != null) {
            requirementsCol.setCellValueFactory(new javafx.scene.control.cell.PropertyValueFactory<>("requirements"));
        }
        if (deadlineCol != null) {
            deadlineCol.setCellValueFactory(new javafx.scene.control.cell.PropertyValueFactory<>("deadline"));
        }
        if (statusCol != null) {
            statusCol.setCellValueFactory(new javafx.scene.control.cell.PropertyValueFactory<>("status"));
        }
        if (appCountCol != null) {
            appCountCol.setCellValueFactory(new javafx.scene.control.cell.PropertyValueFactory<>("applicationCount"));
        }
    }
    ...
    private void loadRecentJobs() {
        ...
        for (Job job : jobList) {
            ...
            String jobType = job.getType() != null ? job.getType().name() : "-";
            String requirements = job.getExtraRequirements() != null ? job.getExtraRequirements() : job.getDescription() != null ? job.getDescription() : "-";
            jobs.add(new JobViewModel(
                    job.getId(),
                    safeText(job.getTitle()),
                    safeText(job.getDepartment()),
                    jobType,
                    truncateText(requirements, 50),
        ...
        if (resultCountLabel != null) {
            resultCountLabel.setText("Results: " + jobs.size() + " positions");
        }
