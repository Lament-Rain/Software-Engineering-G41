...
public class TADashboardController {
    ...
    private void handleApplicationManagement(ActionEvent event) {
        try {
            ...
            util.NavigationUtil.navigateTo(currentStage, "/fxml/TAApplicationHistory.fxml", controller -> {
                if (controller instanceof TAApplicationHistoryController) {
                    ((TAApplicationHistoryController) controller).setUser(user);
                    ((TAApplicationHistoryController) controller).setStage(currentStage);
                }
            });
